...
def main():
    ...
    cold_cur_NDCG_100 = 0
    print(f'len(test_data_gen): {len(test_data_gen)}, len(warm_test_data_gen): {len(warm_test_data_gen)}, len(cold_test_data_gen): {len(cold_test_data_gen)}')
    with torch.no_grad():
        for input_ids, train_mat, target_mat, attention_mask in test_data_loader:
            ...
            if cold_flag:
                # ====== 这里加上 mean/std calibration ======
                warm_scores = item_scores[:, warm_item_idx]  # [batch_size, num_warm_items]
                cold_scores = item_scores[:, cold_item_idx]  # [batch_size, num_cold_items]

                warm_mean = warm_scores.mean(dim=1, keepdim=True)  # [batch_size, 1]
                warm_std = warm_scores.std(dim=1, keepdim=True) + 1e-6 
        ...
        if cold_flag:
            for warm_input_ids, warm_train_mat, warm_target_mat, warm_attention_mask in warm_test_data_loader:
                ...
                warm_scores = warm_item_scores[:, warm_item_idx]  # [batch_size, num_warm_items]
                cold_scores = warm_item_scores[:, cold_item_idx]  # [batch_size, num_cold_items]
            ...
            for cold_input_ids, cold_train_mat, cold_target_mat, cold_attention_mask in cold_test_data_loader:
                ...
                # Set score of interacted items to the lowest
                warm_scores = cold_item_scores[:, warm_item_idx]  # [batch_size, num_warm_items]
                cold_scores = cold_item_scores[:, cold_item_idx]  # [batch_size, num_cold_items]
    ...
    results_path = os.path.join(pretrained_root, f"results_{args.lambda_V}{cold_start_suffix}.txt")
    with fsspec.open(results_path, "w") as f:
        ...
        f.write(f"{warm_cur_recall_20:.4f},{warm_cur_recall_40:.4f},{warm_cur_NDCG_100:.4f}")
        f.write(f"{cold_cur_recall_20:.4f},{cold_cur_recall_40:.4f},{cold_cur_NDCG_100:.4f}")
